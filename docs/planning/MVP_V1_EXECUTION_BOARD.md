...
| `S3-T5` | Define frontend-safe payload guarantees | `P1` | `Done` | `S3-T2`, `S3-T3` | Frontend consumers know which fields are stable, optional, derived, or never public |

### Stage 4 - Instant quote serving maturity

| ID | Ticket | Priority | Status | Depends on | Done when |
|---|---|---|---|---|---|
| `S4-T1` | Freeze the current instant-quote baseline and version contract | `P0` | `Done` | `S3-T3` | There is a named baseline for the existing instant-quote engine and later changes can be compared against it without ambiguity |
| `S4-T2` | Define typed value/tax metadata and response-contract hardening | `P0` | `Done` | `S3-T3`, `S4-T1` | Public and internal consumers can distinguish assessment/value basis, tax basis, and contract-safe optionality without reverse engineering |
| `S4-T3` | Define warning taxonomy and internal/public product-state model | `P0` | `Done` | `S3-T3`, `S4-T2` | Warning states and product/supportability states are explicit and can drive safe behavior without copy drift |
| `S4-T4` | Extend the county capability model for instant-quote supportability | `P0` | `Done` | `S2-T3`, `S4-T2`, `S4-T3` | County-year capability remains one source of truth and now covers quote supportability, suppression, and fallback semantics |
| `S4-T5` | Define `instant_quote_tax_profile` and opportunity-vs-savings separation | `P0` | `Done` | `S4-T2`, `S4-T3`, `S4-T4` | Instant quote distinguishes opportunity from true savings and names the tax-profile contract without pretending a full tax simulator exists |
| `S4-T6` | Define shadow-savings model, launch gates, and feature-flagged serving plan | `P1` | `Done` | `S4-T3`, `S4-T4`, `S4-T5` | Product and ops can compare current vs revised behavior safely before broad rollout |
| `S4-T7` | Define refresh audit, telemetry, calibration, and quote-quality monitoring SOP | `P1` | `Done` | `S4-T4`, `S4-T5`, `S4-T6` | Refresh health, supportability mix, warning mix, and calibration drift can be monitored and escalated consistently |

### Stage 5 - Public quote-to-lead funnel hardening

| ID | Ticket | Priority | Status | Depends on | Done when |
|---|---|---|---|---|---|
| `S5-T1` | Define funnel-state mapping for the public acquisition flow | `P0` | `Done` | none | Quote-ready, unsupported county, unsupported property, missing quote-ready row, and system/config failure each have a clear funnel-state mapping without inventing new quote semantics locally |
| `S5-T2` | Create UX copy matrix and CTA rules for each funnel state | `P0` | `Done` | `S4-T3`, `S4-T5`, `S5-T1` | Every funnel state has approved user messaging and CTA behavior derived from canonical Stage 4 quote states |
...
Execution note:
- keep Stage 6 blocked until the advanced equity valuation engine is intentionally opened as its own workstream
- do not backfill Stage 6 through instant-quote, packet, or funnel tickets

| ID | Ticket | Priority | Status | Depends on | Done when |
|---|---|---|---|---|---|
| `S6-T1` | Define subject-feature normalization and equity comp candidate rules | `P0` | `Blocked` | `S2-T3`, `S4-T5` | Subject parcel inputs and initial comp candidate generation rules are explicit for protest use and are not inferred from instant-quote or packet logic |
...
| `S6-T4` | Implement confidence/supportability scoring and savings-impact logic | `P0` | `Blocked` | `S6-T3` | Internal users can see whether an equity conclusion is strong enough to support downstream protest work without relying on packet or public-quote heuristics |
...
| `S6-T6` | Validate the equity engine on representative Harris and Fort Bend sample data | `P0` | `Blocked` | `S6-T4`, `S6-T5` | The engine produces production-credible protest valuation results on realistic county data, not just fixtures |
