...
- `/admin/ops/jobs/{importBatchId}`
  - batch detail, validation summary, raw files, job runs, publish control, rollback control
...
4. If county automation is weak or a fuller year is needed, use `/admin/ops/manual-upload` to register a manual import.
5. Publish only after validation results and QA surfaces are acceptable.
6. Use rollback only when a published batch needs to be reversed.
7. Review parcel completeness and tax assignment issues before downstream valuation or quote QA.
