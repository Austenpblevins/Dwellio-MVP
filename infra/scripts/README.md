...
- `report_data_readiness.py`: reports county/tax-year raw, canonical, derived readiness, and county capability context for QA year selection and backfill verification.
...
- `report_county_onboarding.py`: builds a repeatable county onboarding contract from capability truth, validation-year ranking, and onboarding gate status.
- `report_scalability_bottlenecks.py`: ranks county-year ingestion and instant-quote bottleneck candidates from persisted telemetry so deeper refactors are evidence-driven instead of speculative.
