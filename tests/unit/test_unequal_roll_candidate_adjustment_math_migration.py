...
def test_unequal_roll_candidate_adjustment_math_migration_present() -> None:
    versions = [migration.version for migration in discover_migrations()]

    assert "0078" in versions


def test_unequal_roll_candidate_adjustment_math_migration_adds_required_structures() -> None:
    migration_path = MIGRATIONS_DIR / "0078_unequal_roll_adjustment_math_scaffolding.sql"
    sql = migration_path.read_text(encoding="utf-8")

    assert "CREATE TABLE IF NOT EXISTS unequal_roll_adjustments" in sql
    assert "ADD COLUMN IF NOT EXISTS adjustment_math_status" in sql
    assert "ADD COLUMN IF NOT EXISTS adjustment_summary_json" in sql
    assert "ADD COLUMN IF NOT EXISTS adjusted_appraised_value" in sql
    assert "unequal_roll_adjustments_adjustment_reliability_flag_check" in sql
    assert "unequal_roll_candidates_adjustment_math_status_check" in sql
    assert "pg_constraint" in sql


def test_unequal_roll_candidate_adjustment_math_migration_uses_env_driven_stage21_db(
    monkeypatch,
) -> None:
...
def test_unequal_roll_candidate_adjustment_math_migration_applies_in_isolated_schema(
    monkeypatch,
) -> None:
    migrations = [
    ...
    schema_name = f"tmp_unequal_roll_adjustment_math_{uuid4().hex[:8]}"
    database_url = _isolated_stage21_database_url(monkeypatch)
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                WHERE table_schema = %s
                  AND table_name = 'unequal_roll_adjustments'
                """,
                (schema_name,),
            )
    ...
    assert "adjustment_math_status" in columns
    assert "adjustment_summary_json" in columns
    assert "adjusted_appraised_value" in columns
    assert "total_absolute_adjustment" in columns


def test_unequal_roll_candidate_adjustment_math_migration_is_idempotent_for_constraints(
    monkeypatch,
) -> None:
    migrations = [
        "0067_unequal_roll_mvp_foundation.sql",
    ...
    schema_name = f"tmp_unequal_roll_adjustment_math_idempotent_{uuid4().hex[:8]}"
    database_url = _isolated_stage21_database_url(monkeypatch)
    ...
    with psycopg.connect(database_url, autocommit=False) as connection:
        with connection.cursor() as cursor:
            ...
            for migration in migrations:
                cursor.execute((MIGRATIONS_DIR / migration).read_text(encoding="utf-8"))
            cursor.execute(
                (MIGRATIONS_DIR / "0078_unequal_roll_adjustment_math_scaffolding.sql").read_text(
                    encoding="utf-8"
                )
            )
        connection.rollback()
