...
def test_unequal_roll_candidate_discovery_migration_present() -> None:
    versions = [migration.version for migration in discover_migrations()]

    assert "0068" in versions


def test_unequal_roll_candidate_discovery_migration_creates_required_table() -> None:
    migration_path = MIGRATIONS_DIR / "0068_unequal_roll_candidate_discovery.sql"
    sql = migration_path.read_text(encoding="utf-8")

    assert "CREATE TABLE IF NOT EXISTS unequal_roll_candidates" in sql
    assert "candidate_parcel_id uuid NOT NULL" in sql
    assert "discovery_tier text NOT NULL" in sql
    assert "candidate_status text NOT NULL DEFAULT 'discovered'" in sql
    assert "source_provenance_json jsonb" in sql
    assert "candidate_snapshot_json jsonb" in sql
    assert "UNIQUE (" in sql


def test_unequal_roll_candidate_discovery_migration_uses_env_driven_stage21_db(
    monkeypatch,
) -> None:
...
def test_unequal_roll_candidate_discovery_migration_applies_in_isolated_schema(
    monkeypatch,
) -> None:
    ...
    schema_name = f"tmp_unequal_roll_candidates_{uuid4().hex[:8]}"
    database_url = _isolated_stage21_database_url(monkeypatch)
    ...
    assert "candidate_parcel_id" in candidate_columns
    assert "discovery_tier" in candidate_columns
    assert "candidate_snapshot_json" in candidate_columns
    assert any("same_neighborhood" in definition for definition in constraint_defs)
    assert any("county_sfr_fallback" in definition for definition in constraint_defs)
