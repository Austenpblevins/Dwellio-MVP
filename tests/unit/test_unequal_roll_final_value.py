...
def test_final_value_refines_stable_no_reduction_review_visible_set_to_supported_with_review(
    monkeypatch,
) -> None:
    values = [
        360000.0,
    ...
    assert result.final_value_status == "supported_with_review"
    assert result.requested_reduction_amount == 0.0
    run_update = _run_update_params(connection)
    final_value_detail = run_update[6].obj
    refinement = final_value_detail["carried_forward_governance"][
        "review_visible_no_reduction_refinement"
    ]
    tiering_refinement = final_value_detail["carried_forward_governance"][
        "no_reduction_governance_tiering_refinement"
    ]
    assert refinement["applied_flag"] is True
    assert refinement["burden_mode"] == "warning_visible"
    assert refinement["checks"]["no_reduction_requested"] is True
    assert refinement["checks"]["source_bath_not_blocking"] is True
    assert refinement["metrics"]["included_count"] == 12
    assert tiering_refinement["applied_flag"] is True


def test_final_value_keeps_positive_reduction_review_visible_set_manual_review_required(
    monkeypatch,
) -> None:
    values = [
        360000.0,
        ...
        fetchall_results=[
            [
                _run_context(
                    selection_governance_status="supported_with_warnings",
                    support_status="supported_with_review",
                    subject_appraised_value=500000.0,
                )
            ],
            candidates,
            adjustment_lines,
    ...
    assert refinement["burden_mode"] == "warning_blocking"
...
def test_final_value_does_not_refine_no_reduction_when_source_is_blocking(
    monkeypatch,
) -> None:
    values = [
        360000.0,
    ...
    candidates = [
        _candidate(
            ...
            review_carry_forward_flag=True,
            adjusted_set_reason_codes=["review_carry_forward_requires_review_visibility"],
            source_status="missing_source_support",
            burden_status="within_thresholds",
            adjustment_pct_of_raw_value=0.03,
            material_adjustment_count=1,
            nontrivial_adjustment_sources_count=1,
    ...
    assert refinement["applied_flag"] is False
    assert refinement["checks"]["source_bath_not_blocking"] is False
    assert refinement["burden_mode"] == "warning_blocking"
