...
def test_apply_savings_translation_rollout_keeps_high_opportunity_state_on_old_path(
    monkeypatch,
) -> None:
    from app.core.config import get_settings

    monkeypatch.setenv("DWELLIO_INSTANT_QUOTE_V5_SAVINGS_TRANSLATION_ENABLED", "true")
    monkeypatch.setenv("DWELLIO_INSTANT_QUOTE_V5_SAVINGS_TRANSLATION_COUNTY_IDS", "harris,fort_bend")
    monkeypatch.setenv(
        "DWELLIO_INSTANT_QUOTE_V5_SAVINGS_TRANSLATION_ROLLOUT_STATES",
    ...
    assert payload["savings_translation_mode"] == "current_public_formula"
    assert payload["savings_translation_applied_flag"] is False
    assert payload["savings_translation_reason_code"] == "rollout_state_not_stage8_calibrated"
    assert updated.estimate is not None
    assert updated.estimate.savings_midpoint_display == response.estimate.savings_midpoint_display
